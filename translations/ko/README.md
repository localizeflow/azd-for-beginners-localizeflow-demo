...
{
  "original_hash": "edf425a0abae35fc6fe849b6cc8c2cd0",
  "translation_date": "2026-01-06T05:04:11+00:00",
  "source_file": "README.md",
  "language_code": "ko"
}
-->
> ⚠️ 이 저장소는 Localizeflow를 사용한 자동화된 문서 현지화 데모를 보여주기 위해 만들어졌습니다.
>
> 원본 내용은
> Microsoft의 “AZD for Beginners” 프로젝트를 기반으로 합니다.
...
3. **커뮤니티 참여하기**: 전문 지원을 위한 [Azure Discord 커뮤니티](https://discord.com/invite/ByRwuEEgH4)
4. **학습 경로 선택하기**: 경험 수준에 맞는 아래 장을 선택하세요
...
구조화된 장을 통해 Azure Developer CLI(azd)를 마스터하세요. **Microsoft Foundry 통합을 통한 AI 애플리케이션 배포에 특별한 중점이 있습니다.**

### 현대 개발자에게 이 과정이 중요한 이유

Microsoft Foundry Discord 커뮤니티 인사이트에 따르면, **개발자의 45%가 AZD를 AI 워크로드에 사용하길 원하지만** 다음과 같은 문제를 겪고 있습니다:
- 복잡한 다중 서비스 AI 아키텍처  
- 프로덕션 AI 배포 모범 사례  
...
- **⚙️ 설정**: [설치 및 셋업](docs/getting-started/installation.md) - 플랫폼별 가이드
- **🛠️ 실습**: [첫 프로젝트](docs/getting-started/first-project.md) - 단계별 튜토리얼
- **📋 요약**: [명령어 치트 시트](resources/cheat-sheet.md)
...
**💡 챕터 목표**: AZD를 사용해 간단한 웹 애플리케이션을 Azure에 성공적으로 배포하기
...
- **🛠️ 워크숍**: [AI 워크숍 실습](docs/microsoft-foundry/ai-workshop-lab.md) - AI 솔루션을 AZD에 적합하게 만들기
- **🎥 인터랙티브 가이드**: [워크숍 자료](workshop/README.md) - 브라우저 기반 학습 및 MkDocs * DevContainer 환경
...
**💡 챕터 목표**: RAG 기능이 있는 AI 채팅 애플리케이션 배포 및 구성
...
# 검색 통합이 작동하는지 확인
azd monitor  # Application Insights가 텔레메트리를 표시하는지 확인
azd down --force --purge
```

**📊 소요 시간:** 1-2시간  
**📈 학습 후 수준:** 프로덕션 준비가 된 AI 애플리케이션을 배포하고 구성 가능  
**💰 비용 인지:** 개발 비용 월 $80-150, 프로덕션 비용 월 $300-3500 이해하기
...
**💡 비용 최적화 팁:**
- 학습 목적에는 **프리 티어** Azure OpenAI 이용 (월 50,000 토큰 포함)
- 개발 중이지 않을 때는 `azd down` 명령어로 리소스 할당 해제
- 프로덕션에는 PTU를 사용하고, 처음에는 종량제 요금제로 시작
- 배포 전에 `azd provision --preview`로 비용 예측
- 자동 확장 활성화: 실제 사용량만큼만 비용 지불
...
#### 학습 자료
- **📖 구성**: [환경 구성 가이드](docs/getting-started/configuration.md)  
- **🔐 보안**: [인증 패턴 및 관리형 ID](docs/getting-started/authsecurity.md)  
...
#### 학습 자료
- **📖 배포**: [배포 가이드](docs/deployment/deployment-guide.md)  
- **🏗️ 프로비저닝**: [리소스 프로비저닝](docs/deployment/provisioning.md)  
...
**💡 챕터 목표**: 맞춤형 인프라 템플릿으로 복잡한 다중 서비스 애플리케이션 배포
...
**💡 장 학습 결과**: 고객 및 재고 에이전트를 포함한 프로덕션 준비된 다중 에이전트 AI 솔루션 배포 및 관리
...
**💡 장 학습 결과**: 배포 실행 전 검증 및 최적화 수행

---

### 🚨 7장: 문제 해결 및 디버깅  
**필수 조건**: 배포 관련 장 중 하나 완료  
**소요 시간**: 1-1.5시간  
**난이도**: ⭐⭐
...
- **🤖 AI 문제**: [AI 문제 해결](docs/troubleshooting/ai-troubleshooting.md) - AI 서비스 이슈
...
**💡 장 학습 결과**: 일반 배포 문제를 독립적으로 진단 및 해결
...
**💡 장 학습 결과**: 완전한 프로덕션 기능을 갖춘 엔터프라이즈용 앱 배포
...
> **⚠️ 워크숍 상태: 개발 중**  
> 워크숍 자료는 현재 개발 및 개선 중입니다. 핵심 모듈은 작동하지만 일부 고급 섹션은 미완성입니다. 전체 콘텐츠 완성을 위해 적극 작업 중입니다. [진행 상황 보기 →](workshop/README.md)

### 인터랙티브 워크숍 자료  
**브라우저 기반 도구와 안내된 실습을 통한 종합 실습 학습**

본 워크숍 자료는 위 장별 커리큘럼을 보완하는 체계적이고 인터랙티브한 학습 경험을 제공합니다. 워크숍은 자율 학습 및 강사 주도 세션 모두에 적합하게 설계되었습니다.

#### 🛠️ 워크숍 특징
- **브라우저 기반 인터페이스**: 검색, 복사, 테마 기능이 포함된 MkDocs 기반 워크숍
- **GitHub Codespaces 통합**: 원클릭 개발 환경 설정
- **체계적 학습 경로**: 7단계 안내 실습 (총 3.5시간)
- **발견 → 배포 → 맞춤화**: 점진적 학습 방법론
- **인터랙티브 DevContainer 환경**: 사전 구성된 도구 및 종속성 포함
...
- **📋 단계별 안내**: [안내 실습](../../workshop/docs/instructions) - 자세한 단계별 설명  
...
**추천 대상**: 기업 교육, 대학 강의, 자율 학습, 개발자 부트캠프

---

## 📖 Azure Developer CLI란?

Azure Developer CLI(azd)는 개발자 중심 명령줄 인터페이스로, Azure에 애플리케이션을 구축하고 배포하는 과정을 가속화합니다. 주요 기능은 다음과 같습니다:

- **템플릿 기반 배포** - 공통 애플리케이션 패턴을 위한 사전 구축 템플릿 사용  
- **코드로서의 인프라** - Bicep 또는 Terraform을 통한 Azure 리소스 관리  
- **통합 워크플로우** - 애플리케이션 프로비저닝, 배포, 모니터링의 원활한 연계  
- **개발자 친화적** - 개발 생산성과 경험을 최적화
...
> **참고:** 이 템플릿들은 다양한 AI 패턴을 보여줍니다. 일부는 외부 Azure 샘플, 일부는 로컬 구현입니다.
...
| [**AI 에이전트 시작하기**](https://github.com/Azure-Samples/get-started-with-ai-agents) | 2장 | ⭐⭐ | Azure AI 에이전트 서비스 + AzureOpenAI + Azure AI 검색 + Azure 컨테이너 앱 + Application Insights | 외부 |
| [**Azure 검색 + OpenAI 데모**](https://github.com/Azure-Samples/azure-search-openai-demo) | 2장 | ⭐⭐ | AzureOpenAI + Azure AI 검색 + 앱 서비스 + 스토리지 | 외부 |
| [**OpenAI 채팅 앱 퀵스타트**](https://github.com/Azure-Samples/openai-chat-app-quickstart) | 2장 | ⭐ | AzureOpenAI + 컨테이너 앱 + Application Insights | 외부 |
| [**에이전트 OpenAI Python Prompty**](https://github.com/Azure-Samples/agent-openai-python-prompty) | 5장 | ⭐⭐⭐ | AzureOpenAI + Azure Functions + Prompty | 외부 |
...
> **외부 예제**(Azure 샘플) = 연동된 저장소에서 클론하여 사용

#### 로컬 예제 (즉시 사용 가능)
- [**리테일 다중 에이전트 솔루션**](examples/retail-scenario.md) - ARM 템플릿과 함께 완전한 프로덕션 준비 구현  
  - 다중 에이전트 아키텍처 (고객 + 재고 에이전트)  
  - 포괄적 모니터링 및 평가  
  - ARM 템플릿을 통한 원클릭 배포  

#### 로컬 예제 - 컨테이너 앱 (2~5장)  
**이 저장소의 종합 컨테이너 배포 예제:**  
- [**컨테이너 앱 예제**](examples/container-app/README.md) - 컨테이너화 배포 완전 안내서  
  - [간단한 Flask API](../../examples/container-app/simple-flask-api) - scale-to-zero이 포함된 기본 REST API  
  - [마이크로서비스 아키텍처](../../examples/container-app/microservices) - 프로덕션 준비 다중 서비스 배포  
  - 빠른 시작, 프로덕션 및 고급 배포 패턴  
  - 모니터링, 보안 및 비용 최적화 가이드  

#### 외부 예제 - 간단 앱 (1~2장)  
**아래 Azure 샘플 저장소를 클론하여 시작하세요:**  
- [간단 웹 앱 - Node.js + MongoDB](https://github.com/Azure-Samples/todo-nodejs-mongo) - 기본 배포 패턴  
- [정적 웹 사이트 - React SPA](https://github.com/Azure-Samples/todo-csharp-sql-swa-func) - 정적 콘텐츠 배포  
...
### 외부 템플릿 컬렉션
- [**공식 AZD 템플릿 갤러리**](https://azure.github.io/awesome-azd/) - 공식 및 커뮤니티 템플릿의 엄선된 모음
...
### 빠른 참고 자료
- [**명령어 요약표**](resources/cheat-sheet.md) - 장별로 정리된 필수 azd 명령어
- [**용어집**](resources/glossary.md) - Azure 및 azd 용어  
- [**자주 묻는 질문(FAQ)**](resources/faq.md) - 학습 장별 자주 묻는 질문
- [**학습 가이드**](resources/study-guide.md) - 종합 연습 문제

### 실습 워크숍
- [**AI 워크숍 랩**](docs/microsoft-foundry/ai-workshop-lab.md) - AI 솔루션의 AZD 배포 가능성 확보 (2-3시간)
- [**인터랙티브 워크숍 가이드**](workshop/README.md) - MkDocs 및 DevContainer 환경 기반 브라우저 워크숍
- [**구조화된 학습 경로**](../../workshop/docs/instructions) - 7단계 안내형 실습 (탐색 → 배포 → 맞춤화)
- [**초보자를 위한 AZD 워크숍**](workshop/README.md) - GitHub Codespaces 통합의 완전한 실습 워크숍 자료
...
# infra/main.parameters.json 파일을 수정하세요:
{
...
- **공통 문제 가이드:** [상세 해결책](docs/troubleshooting/common-issues.md)
...
### 학습 검증
각 장 완료 후 지식을 검증하세요:
1. **실습 과제**: 해당 장의 실전 배포 완료
2. **지식 점검**: 장별 FAQ 섹션 검토
3. **커뮤니티 토론**: Azure Discord에서 경험 공유
4. **다음 장 진행**: 다음 난이도 단계로 이동

### 과정 완료 혜택
모든 장을 완료하면 다음을 갖출 수 있습니다:
...
- **AI 전용 도움말**: [![Microsoft Foundry Discord](https://dcbadge.limes.pink/api/server/nTYy5BXMWG)](https://discord.gg/nTYy5BXMWG) 가입
...
**#Azure 채널 최근 설문 결과:**
- **45%** 개발자가 AI 작업에 AZD 사용 희망
- **주요 도전과제**: 다중 서비스 배포, 자격 증명 관리, 운영 준비  
...
- **콘텐츠 개선**: 기존 장 및 예제 향상
- **새 예제 추가**: 실무 시나리오 및 템플릿 추가  
- **번역**: 다국어 지원 유지 보수
- **버그 리포트**: 정확도 및 명확성 향상
- **커뮤니티 기준**: 포괄적 커뮤니티 가이드 준수
...
### 라이선스
본 프로젝트는 MIT 라이선스 하에 제공됩니다 - 자세한 내용은 [LICENSE](../../LICENSE) 파일 참조.

### 관련 Microsoft 학습 자료

저희 팀은 다른 포괄적인 학습 과정을 제작하고 있습니다:
...
**면책 조항**:  
이 문서는 AI 번역 서비스 [Co-op Translator](https://github.com/Azure/co-op-translator)를 사용하여 번역되었습니다. 저희는 정확성을 위해 최선을 다하고 있으나, 자동 번역은 오류나 부정확한 부분을 포함할 수 있음을 양지해 주시기 바랍니다. 원문은 해당 언어의 권위 있는 출처로 간주되어야 합니다. 중요한 정보의 경우 전문 인력에 의한 번역을 권장합니다. 본 번역 사용으로 인한 오해나 잘못된 해석에 대해 당사는 책임을 지지 않습니다.
